...
# ---------------------------------------------------------------------------
# [TEST] Сценарии подписки — удалить после проверки
# ---------------------------------------------------------------------------

async def _test_redirect(msg: str) -> RedirectResponse:
    from urllib.parse import quote
    return RedirectResponse(url=f"/admin?success={quote(msg)}", status_code=302)


@router.post("/user/{user_id}/test/activate")
async def test_activate(
    request: Request, response: Response, user_id: int, db: AsyncSession = Depends(get_db),
):
    """simple → premium: активация подписки (первый раз)."""
    if not await _check_admin(request, response, db):
        raise HTTPException(status_code=403)
    ...
    duration_days = settings_cache.get_int("premium_duration_days")
    user.role = "premium"
    user.premium_until = (now + timedelta(days=duration_days)).replace(hour=23, minute=59, second=59, microsecond=0)
    user.timecode_grace_until = None
    user.premium_warned = False
    ...
    if user.notifications_enabled:
        from app.db.models import TelegramUser
        from app.tasks import _send_premium_activated
        tg = (await db.execute(select(TelegramUser).where(TelegramUser.user_id == user_id))).scalar_one_or_none()
        if tg:
            await _send_premium_activated(tg.telegram_id, user)

    return await _test_redirect(f"{user.username}: подписка активирована до {user.premium_until.strftime('%d.%m.%Y')}")


@router.post("/user/{user_id}/test/revoke")
async def test_revoke(
    request: Request, response: Response, user_id: int, db: AsyncSession = Depends(get_db),
):
    """Удалить подписку → simple, без уведомления (сброс для тестов)."""
    if not await _check_admin(request, response, db):
        raise HTTPException(status_code=403)
    ...
    return await _test_redirect(f"{user.username}: подписка удалена → simple")


@router.post("/user/{user_id}/test/warning")
async def test_warning(
    request: Request, response: Response, user_id: int, db: AsyncSession = Depends(get_db),
):
    """Установить premium_until = now+2д, запустить expiry check → предупреждение придёт через delivery."""
    if not await _check_admin(request, response, db):
        raise HTTPException(status_code=403)
    ...
    now = datetime.now(timezone.utc)
    user.role = "premium"
    user.premium_until = (now + timedelta(days=2)).replace(hour=23, minute=59, second=59, microsecond=0)
    user.premium_warned = False
    user.timecode_grace_until = None
    user.notify_premium_after = None
    user.notify_type = None
    ...
    from app.tasks import run_premium_expiry_check, run_notification_delivery
    await run_premium_expiry_check(_now=now)
    await run_notification_delivery()

    return await _test_redirect(f"{user.username}: запущен expiry check (warning), уведомление отправлено")


@router.post("/user/{user_id}/test/extend")
async def test_extend(
    request: Request, response: Response, user_id: int, db: AsyncSession = Depends(get_db),
):
    """Продлить активный premium на 30 дней + уведомление о продлении."""
    if not await _check_admin(request, response, db):
        raise HTTPException(status_code=403)
    ...
    return await _test_redirect(f"{user.username}: подписка продлена до {user.premium_until.strftime('%d.%m.%Y')}")


@router.post("/user/{user_id}/test/restore")
async def test_restore(
    request: Request, response: Response, user_id: int, db: AsyncSession = Depends(get_db),
):
    """Симулировать grace → восстановление: ставит simple+grace, затем сразу premium+уведомление."""
    if not await _check_admin(request, response, db):
        raise HTTPException(status_code=403)
    ...
    # Шаг 1: симулируем состояние grace
    user.role = "simple"
    user.premium_until = None
    user.timecode_grace_until = now + timedelta(days=1)  # в grace-периоде
    await db.commit()

    # Шаг 2: восстанавливаем premium
    user.role = "premium"
    user.premium_until = (now + timedelta(days=duration_days)).replace(hour=23, minute=59, second=59, microsecond=0)
    user.timecode_grace_until = None
    user.premium_warned = False
    ...
    return await _test_redirect(f"{user.username}: симуляция восстановления из grace выполнена")


@router.post("/user/{user_id}/test/expire")
async def test_expire(
    request: Request, response: Response, user_id: int, db: AsyncSession = Depends(get_db),
):
    """Установить premium_until в прошлое, запустить expiry check → понижение + уведомление + очистка."""
    if not await _check_admin(request, response, db):
        raise HTTPException(status_code=403)
    ...
    now = datetime.now(timezone.utc)
    user.role = "premium"
    user.premium_until = now - timedelta(seconds=1)  # только что истекло
    ...
    # Перечитываем для отображения финального состояния
    await db.refresh(user)
    grace_str = user.timecode_grace_until.strftime('%d.%m.%Y') if user.timecode_grace_until else "нет"
    return await _test_redirect(
        f"{user.username}: premium истёк → simple, grace до {grace_str}, уведомление отправлено"
    )
